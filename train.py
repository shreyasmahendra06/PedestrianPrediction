if __name__ =='__main__':
    ...
    class args():
        def __init__(self):
            self.jaad_dataset = 'D:/bounding-box-prediction/JAAD/processed_annotations' #folder containing parsed jaad annotations (used when first time loading data)
            self.dtype        = 'train'
            self.from_file    = False #read dataset from csv file or reprocess data
            self.save         = True
            self.file         = 'D:/bounding-box-prediction/JAAD/jaad_train_16_16.csv'
            self.save_path    = 'D:/bounding-box-prediction/JAAD/jaad_train_16_16.csv'
            self.model_path    = 'D:/bounding-box-prediction/models/multitask_pv_lstm_trained.pkl'
    ...
    for epoch in range(args.n_epochs):
        ...
        for idx, (obs_s, target_s, obs_p, target_p, target_c, label_c) in enumerate(train):
            counter += 1
            obs_s    = obs_s.to(device='cpu')
            target_s = target_s.to(device='cpu')
            obs_p    = obs_p.to(device='cpu')
            target_p = target_p.to(device='cpu')
            target_c = target_c.to(device='cpu')
