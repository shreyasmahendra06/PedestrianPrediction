...
def AIOU(pred, true):
    ...
    interArea = torch.max(torch.zeros(min_inter.shape[0],min_inter.shape[1]).to('cpu'), (max_inter[:,:,0]-min_inter[:,:,0])) *\
                torch.max(torch.zeros(max_inter.shape[0],max_inter.shape[1]).to('cpu'), (max_inter[:,:,1]-min_inter[:,:,1]))
...
def FIOU(pred, true):
    ...
    interArea = torch.max(torch.zeros(min_inter.shape[0]).to('cpu'), (max_inter[:,0]-min_inter[:,0])) * \
                torch.max(torch.zeros(max_inter.shape[0]).to('cpu'), (max_inter[:,1]-min_inter[:,1]))
...
def speed2pos(preds, obs_p):
    pred_pos = torch.zeros(preds.shape[0], preds.shape[1], 4).to('cpu')
    current = obs_p[:,-1,:]
    for i in range(preds.shape[1]):
        pred_pos[:,i,:] = current + preds[:,i,:]
        current = pred_pos[:,i,:]
        
    pred_pos[:,:,0] = torch.min(pred_pos[:,:,0], 1920*torch.ones(pred_pos.shape[0], pred_pos.shape[1], device='cpu'))
    pred_pos[:,:,1] = torch.min(pred_pos[:,:,1], 1080*torch.ones(pred_pos.shape[0], pred_pos.shape[1], device='cpu'))
    pred_pos[:,:,0] = torch.max(pred_pos[:,:,0], torch.zeros(pred_pos.shape[0], pred_pos.shape[1], device='cpu'))
    pred_pos[:,:,1] = torch.max(pred_pos[:,:,1], torch.zeros(pred_pos.shape[0], pred_pos.shape[1], device='cpu'))
